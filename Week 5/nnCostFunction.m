...
X = [ones(m, 1) X];
SigmoidTwo = sigmoid(X*Theta1');
SigmoidTwo = [ones(m,1) SigmoidTwo];
SigmoidThree = sigmoid(SigmoidTwo*Theta2');
y_matrix=eye(num_labels)(y,:);
J=sum(sum((-1*(y_matrix.*log(SigmoidThree)+(1-y_matrix).*log(1-SigmoidThree)))))/m;

J = J + (sum(sum(Theta1(:,2:size(Theta1,2)).^2))+sum(sum(Theta2(:,2:size(Theta2,2)).^2)))*lambda/(2*m);

d3 = SigmoidThree-y_matrix;
